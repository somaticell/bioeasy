...
public abstract class XYChart extends AbstractChart {
    ...
    public void draw(Canvas canvas, int x, int y, int width, int height, Paint paint) {
        ...
        int bottom2 = ((y + height) - margins[2]) - legendSize;
        if (this.mScreenR == null) {
            this.mScreenR = new Rect();
        }
        this.mScreenR.set(left, top, right, bottom2);
        ...
        if (maxScaleNumber2 >= 0) {
            ...
            for (int i6 = 0; i6 < sLength; i6++) {
                if (this.mDataset.getSeriesAt(i6).getItemCount() != 0) {
                    hasValues = true;
                }
            }
            ...
            if (showGridX || showGridY) {
                ...
                this.mRenderer.setShowLabels(showXLabels, showYLabels);
            }
            this.clickableAreas = new HashMap();
            for (int i7 = 0; i7 < sLength; i7++) {
                XYSeries series2 = this.mDataset.getSeriesAt(i7);
                int scale2 = series2.getScaleNumber();
                if (series2.getItemCount() != 0) {
                    XYSeriesRenderer seriesRenderer = (XYSeriesRenderer) this.mRenderer.getSeriesRendererAt(i7);
                    ...
                    this.clickableAreas.put(Integer.valueOf(i7), clickableArea);
                    synchronized (series2) {
                        ...
                        for (Map.Entry<Double, Double> value : series2.getRange(minX[scale2], maxX[scale2], seriesRenderer.isDisplayBoundingPoints()).entrySet()) {
                            ...
                            if (!isNullValue(yValue)) {
                                ...
                                if (arrayList.size() > 0) {
                                    drawSeries(series2, canvas, paint, arrayList, seriesRenderer, yAxisValue, i7, or, startIndex);
                                    clickableArea.addAll(Arrays.asList(clickableAreasForPoints(arrayList, arrayList2, yAxisValue, i7, startIndex)));
                                    arrayList.clear();
                                    arrayList2.clear();
                                    startIndex = -1;
                                }
            ...
            if (showLabels) {
                List<Double> xLabels2 = getValidLabels(getXLabels(minX[0], maxX[0], this.mRenderer.getXLabels()));
                Map<Integer, List<Double>> allYLabels2 = getYLabels(minY, maxY, maxScaleNumber2);
                int xLabelsLeft2 = left;
                if (showLabels) {
                ...
                drawXLabels(xLabels2, this.mRenderer.getXTextLabelLocations(), canvas, paint, xLabelsLeft2, top, bottom, xPixelsPerUnit[0], minX[0], maxX[0]);
                drawYLabels(allYLabels2, canvas, paint, maxScaleNumber2, left, right, bottom, yPixelsPerUnit, minY);
                this.mRenderer.setShowGridX(showGridX);
                this.mRenderer.setShowGridY(showGridY);
                if (showLabels) {
                    paint.setColor(this.mRenderer.getLabelsColor());
                    for (int i8 = 0; i8 < maxScaleNumber2; i8++) {
                        Paint.Align axisAlign = this.mRenderer.getYAxisAlign(i8);
                        Double[] arr$ = this.mRenderer.getYTextLabelLocations(i8);
                        int len$ = arr$.length;
                        for (int i$ = 0; i$ < len$; i$++) {
                            Double location = arr$[i$];
                            if (minY[i8] <= location.doubleValue() && location.doubleValue() <= maxY[i8]) {
                                float yLabel = (float) (((double) bottom) - (yPixelsPerUnit[i8] * (location.doubleValue() - minY[i8])));
                                ...
                                if (or == XYMultipleSeriesRenderer.Orientation.HORIZONTAL) {
                                    if (axisAlign == Paint.Align.LEFT) {
                                        ...
                                        drawText(canvas, label, ((float) left) - this.mRenderer.getYLabelsPadding(), yLabel - this.mRenderer.getYLabelsVerticalPadding(), paint, this.mRenderer.getYLabelsAngle());
                                    } else {
                                        if (showTickMarks) {
                                            canvas.drawLine((float) right, yLabel, (float) (getLabelLinePos(axisAlign) + right), yLabel, paint);
                                        }
                                        drawText(canvas, label, ((float) right) - this.mRenderer.getYLabelsPadding(), yLabel - this.mRenderer.getYLabelsVerticalPadding(), paint, this.mRenderer.getYLabelsAngle());
                                    }
                ...
                if (showLabels) {
                    ...
                    if (or == XYMultipleSeriesRenderer.Orientation.HORIZONTAL) {
                        ...
                        for (int i9 = 0; i9 < maxScaleNumber2; i9++) {
                            if (this.mRenderer.getYAxisAlign(i9) == Paint.Align.LEFT) {
                                drawText(canvas, this.mRenderer.getYTitle(i9), ((float) x) + size, (float) ((height / 2) + y), paint, -90.0f);
                            } else {
                                drawText(canvas, this.mRenderer.getYTitle(i9), (float) (x + width), (float) ((height / 2) + y), paint, -90.0f);
                            }
                        }
            ...
            if (this.mRenderer.isShowAxes()) {
                ...
                for (int i10 = 0; i10 < maxScaleNumber2 && !rightAxis; i10++) {
    ...
    public void drawChartValuesText(Canvas canvas, XYSeries series, XYSeriesRenderer renderer, Paint paint, List<Float> points, int seriesIndex, int startIndex) {
        if (points.size() > 2) {
            ...
            for (int k = 0; k < points.size(); k += 2) {
                if (k == 2) {
                    ...
                    drawText(canvas, getLabel(renderer.getChartValuesFormat(), series.getY((k / 2) + startIndex)), points.get(k).floatValue(), points.get(k + 1).floatValue() - renderer.getChartValuesSpacing(), paint, 0.0f);
        ...
        for (int k2 = 0; k2 < points.size(); k2 += 2) {
            drawText(canvas, getLabel(renderer.getChartValuesFormat(), series.getY((k2 / 2) + startIndex)), points.get(k2).floatValue(), points.get(k2 + 1).floatValue() - renderer.getChartValuesSpacing(), paint, 0.0f);
        }
    }
    ...
    public void drawXLabels(List<Double> xLabels, Double[] xTextLabelLocations, Canvas canvas, Paint paint, int left, int top, int bottom, double xPixelsPerUnit, double minX, double maxX) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (showXLabels) {
                ...
                float f = xLabel;
                Paint paint2 = paint;
                drawText(canvas2, getLabel(this.mRenderer.getXLabelFormat(), label), f, ((float) bottom) + ((this.mRenderer.getLabelsTextSize() * 4.0f) / 3.0f) + this.mRenderer.getXLabelsPadding(), paint2, this.mRenderer.getXLabelsAngle());
            }
            if (showGridY) {
        ...
        drawXTextLabels(xTextLabelLocations, canvas, paint, showXLabels, left, top, bottom, xPixelsPerUnit, minX, maxX);
    }
                ...
                if (or == XYMultipleSeriesRenderer.Orientation.HORIZONTAL) {
                    if (showYLabels && !textLabel) {
                        ...
                        if (axisAlign == Paint.Align.LEFT) {
                            ...
                            drawText(canvas, getLabel(this.mRenderer.getYLabelFormat(i), label), ((float) left) - this.mRenderer.getYLabelsPadding(), yLabel - this.mRenderer.getYLabelsVerticalPadding(), paint, this.mRenderer.getYLabelsAngle());
                        } else {
                            if (showTickMarks) {
                                canvas.drawLine((float) right, yLabel, (float) (getLabelLinePos(axisAlign) + right), yLabel, paint);
                            }
                            drawText(canvas, getLabel(this.mRenderer.getYLabelFormat(i), label), ((float) right) + this.mRenderer.getYLabelsPadding(), yLabel - this.mRenderer.getYLabelsVerticalPadding(), paint, this.mRenderer.getYLabelsAngle());
                        }
                    }
                    if (showGridX) {
        ...
        if (showLabels) {
            ...
            for (Double location : xTextLabelLocations) {
                if (minX <= location.doubleValue() && location.doubleValue() <= maxX) {
                    ...
                    drawText(canvas, this.mRenderer.getXTextLabel(location), xLabel, ((float) bottom) + ((this.mRenderer.getLabelsTextSize() * 4.0f) / 3.0f) + this.mRenderer.getXLabelsPadding(), paint, this.mRenderer.getXLabelsAngle());
