...
public final class Buffer implements BufferedSource, BufferedSink, Cloneable {
        ...
        return new InputStream() {
            public int read() {
                if (Buffer.this.size > 0) {
                    return Buffer.this.readByte() & BLEServiceApi.CONNECTED_STATUS;
                }
                return -1;
            }
    ...
    public short readShort() {
        ...
        int limit = segment.limit;
        if (limit - pos < 2) {
            return (short) (((readByte() & BLEServiceApi.CONNECTED_STATUS) << 8) | (readByte() & 255));
        }
        byte[] data = segment.data;
        int pos2 = pos + 1;
        int pos3 = pos2 + 1;
        int s = ((data[pos] & BLEServiceApi.CONNECTED_STATUS) << 8) | (data[pos2] & 255);
        this.size -= 2;
        if (pos3 == limit) {
            this.head = segment.pop();
            SegmentPool.recycle(segment);
    ...
    public int readInt() {
        ...
        int limit = segment.limit;
        if (limit - pos < 4) {
            return ((readByte() & BLEServiceApi.CONNECTED_STATUS) << 24) | ((readByte() & BLEServiceApi.CONNECTED_STATUS) << 16) | ((readByte() & BLEServiceApi.CONNECTED_STATUS) << 8) | (readByte() & BLEServiceApi.CONNECTED_STATUS);
        }
        byte[] data = segment.data;
        int pos2 = pos + 1;
        int pos3 = pos2 + 1;
        int pos4 = pos3 + 1;
        int pos5 = pos4 + 1;
        byte b = ((data[pos] & BLEServiceApi.CONNECTED_STATUS) << 24) | ((data[pos2] & BLEServiceApi.CONNECTED_STATUS) << 16) | ((data[pos3] & BLEServiceApi.CONNECTED_STATUS) << 8) | (data[pos4] & BLEServiceApi.CONNECTED_STATUS);
        this.size -= 4;
        if (pos5 == limit) {
            this.head = segment.pop();
            SegmentPool.recycle(segment);
