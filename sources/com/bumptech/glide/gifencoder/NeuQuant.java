...
class NeuQuant {
    ...
    public void learn() {
        ...
        while (i2 < samplepixels) {
            int b = (p[pix + 0] & BLEServiceApi.CONNECTED_STATUS) << 4;
            int g = (p[pix + 1] & BLEServiceApi.CONNECTED_STATUS) << 4;
            int r = (p[pix + 2] & BLEServiceApi.CONNECTED_STATUS) << 4;
            int j = contest(b, g, r);
    ...
    public int contest(int b, int g, int r) {
        int bestd = ActivityChooserView.ActivityChooserViewAdapter.MAX_ACTIVITY_COUNT_UNLIMITED;
