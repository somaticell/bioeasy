...
public class EditPageLand extends EditPage implements View.OnClickListener, TextWatcher, Runnable {
    ...
    public void onCreate() {
        super.onCreate();
        float ratio = ((float) ResHelper.getScreenHeight(this.activity)) / 720.0f;
        ...
        this.llPage.addView(this.rlTitle, new LinearLayout.LayoutParams(-1, (int) (70.0f * ratio)));
        initTitle(this.rlTitle, ratio);
        RelativeLayout rlBody = new RelativeLayout(this.activity);
        rlBody.setBackgroundColor(-1);
        this.llPage.addView(rlBody, new LinearLayout.LayoutParams(-1, -2));
    ...
    private void initTitle(RelativeLayout rlTitle, float ratio) {
        ...
        int resId2 = ResHelper.getStringRes(this.activity, "ssdk_oks_multi_share");
        if (resId2 > 0) {
        ...
        int resId3 = ResHelper.getStringRes(this.activity, "ssdk_oks_share");
        if (resId3 > 0) {
            this.tvShare.setText(resId3);
        }
        this.tvShare.setPadding(padding, 0, padding, 0);
        RelativeLayout.LayoutParams lp2 = new RelativeLayout.LayoutParams(-2, -1);
        lp2.addRule(11);
        rlTitle.addView(this.tvShare, lp2);
    ...
    private void initBody(RelativeLayout rlBody, float ratio) {
        ...
        lp.weight = 1.0f;
        llContent.addView(this.etContent, lp);
        ...
        LinearLayout.LayoutParams lp2 = new LinearLayout.LayoutParams(thumbWidth, thumbWidth);
        lp2.topMargin = padding;
        lp2.bottomMargin = padding;
        lp2.rightMargin = padding;
        llContent.addView(this.rlThumb, lp2);
        this.aivThumb = new AsyncImageView(this.activity) {
            public void onImageGot(String url, Bitmap bm) {
                Bitmap unused = EditPageLand.this.thumb = bm;
    ...
    private void initBottom(LinearLayout llBottom, float ratio) {
        ...
        llBottom.addView(llAt, new LinearLayout.LayoutParams(-1, (int) (75.0f * ratio)));
        ...
        llAt.addView(this.tvAt, new LinearLayout.LayoutParams(-2, -1));
        ...
        v.setBackgroundColor(DefaultRenderer.TEXT_COLOR);
        llBottom.addView(v, new LinearLayout.LayoutParams(-1, ratio > 1.0f ? (int) ratio : 1));
    }

    private void initShadow(LinearLayout llShadow, float ratio) {
        LinearLayout.LayoutParams lp = new LinearLayout.LayoutParams(-1, ratio > 1.0f ? (int) ratio : 1);
        View v = new View(this.activity);
        v.setBackgroundColor(687865856);
        llShadow.addView(v, lp);
        View v2 = new View(this.activity);
        v2.setBackgroundColor(335544320);
        llShadow.addView(v2, lp);
        View v3 = new View(this.activity);
        v3.setBackgroundColor(117440512);
        llShadow.addView(v3, lp);
    }
