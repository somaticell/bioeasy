...
public final class Spdy3 implements Variant {
    ...
    static final class Reader implements FrameReader {
        ...
        private void readHeaders(FrameReader.Handler handler, int flags, int length) throws IOException {
            handler.headers(false, false, this.source.readInt() & ActivityChooserView.ActivityChooserViewAdapter.MAX_ACTIVITY_COUNT_UNLIMITED, -1, this.headerBlockReader.readNameValueBlock(length - 4), HeadersMode.SPDY_HEADERS);
        }
        ...
        private void readWindowUpdate(FrameReader.Handler handler, int flags, int length) throws IOException {
            ...
            int w2 = this.source.readInt();
            int streamId = w1 & ActivityChooserView.ActivityChooserViewAdapter.MAX_ACTIVITY_COUNT_UNLIMITED;
            long increment = (long) (w2 & ActivityChooserView.ActivityChooserViewAdapter.MAX_ACTIVITY_COUNT_UNLIMITED);
            if (increment == 0) {
        ...
        private void readGoAway(FrameReader.Handler handler, int flags, int length) throws IOException {
            ...
            int lastGoodStreamId = this.source.readInt() & ActivityChooserView.ActivityChooserViewAdapter.MAX_ACTIVITY_COUNT_UNLIMITED;
            int errorCodeInt = this.source.readInt();
        ...
        private void readSettings(FrameReader.Handler handler, int flags, int length) throws IOException {
            ...
            for (int i = 0; i < numberOfEntries; i++) {
                int w1 = this.source.readInt();
                settings.set(w1 & ViewCompat.MEASURED_SIZE_MASK, (-16777216 & w1) >>> 24, this.source.readInt());
            }
            if ((flags & 1) == 0) {
                clearPrevious = false;
            }
