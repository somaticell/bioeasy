...
final class Base64 {
    ...
    private static String encode(byte[] in, byte[] map) {
        ...
        for (int i = 0; i < end; i += 3) {
            int index3 = index2 + 1;
            out[index2] = map[(in[i] & BLEServiceApi.CONNECTED_STATUS) >> 2];
            int index4 = index3 + 1;
            out[index3] = map[((in[i] & 3) << 4) | ((in[i + 1] & BLEServiceApi.CONNECTED_STATUS) >> 4)];
            int index5 = index4 + 1;
            out[index4] = map[((in[i + 1] & 15) << 2) | ((in[i + 2] & BLEServiceApi.CONNECTED_STATUS) >> 6)];
            index2 = index5 + 1;
        ...
        switch (in.length % 3) {
            case 1:
                int index6 = index2 + 1;
                out[index2] = map[(in[end] & BLEServiceApi.CONNECTED_STATUS) >> 2];
                int index7 = index6 + 1;
            ...
            case 2:
                ...
                out[index9] = map[((in[end] & 3) << 4) | ((in[end + 1] & BLEServiceApi.CONNECTED_STATUS) >> 4)];
