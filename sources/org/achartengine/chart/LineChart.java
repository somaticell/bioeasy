        ...
        for (XYSeriesRenderer.FillOutsideLine fill : renderer.getFillOutsideLine()) {
            if (fill.getType() != XYSeriesRenderer.FillOutsideLine.Type.NONE) {
                ...
                if (fill.getType() == XYSeriesRenderer.FillOutsideLine.Type.BOUNDS_ABOVE || fill.getType() == XYSeriesRenderer.FillOutsideLine.Type.BOUNDS_BELOW) {
                    ...
                    if ((length > 0 && fill.getType() == XYSeriesRenderer.FillOutsideLine.Type.BOUNDS_ABOVE && fillPoints.get(1).floatValue() < referencePoint) || (fill.getType() == XYSeriesRenderer.FillOutsideLine.Type.BOUNDS_BELOW && fillPoints.get(1).floatValue() > referencePoint)) {
                ...
                if (length2 > 0) {
                    ...
                    fillPoints.add(fillPoints.get(length2 - 2));
                    fillPoints.add(Float.valueOf(referencePoint));
                    fillPoints.add(fillPoints.get(0));
                    fillPoints.add(fillPoints.get(length2 + 1));
                    for (int i2 = 0; i2 < length2 + 4; i2 += 2) {
                        if (fillPoints.get(i2 + 1).floatValue() < 0.0f) {
                            fillPoints.set(i2 + 1, Float.valueOf(0.0f));
                        }
                    }
