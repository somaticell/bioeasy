...
public final class SerializeWriter extends Writer {
    ...
    public void writeByteArray(byte[] bytes) {
        ...
        if (newcount > this.buf.length) {
            if (this.writer != null) {
                ...
                while (true) {
                    ...
                    int s4 = s3 + 1;
                    s = s4 + 1;
                    int i = ((bytes[s2] & BLEServiceApi.CONNECTED_STATUS) << 16) | ((bytes[s3] & BLEServiceApi.CONNECTED_STATUS) << 8) | (bytes[s4] & 255);
                ...
                if (left > 0) {
                    int i2 = ((bytes[eLen] & BLEServiceApi.CONNECTED_STATUS) << 10) | (left == 2 ? (bytes[bytesLen - 1] & BLEServiceApi.CONNECTED_STATUS) << 2 : 0);
                    write((int) CA[i2 >> 12]);
        ...
        while (true) {
            ...
            int s8 = s7 + 1;
            s5 = s8 + 1;
            int i3 = ((bytes[s6] & BLEServiceApi.CONNECTED_STATUS) << 16) | ((bytes[s7] & BLEServiceApi.CONNECTED_STATUS) << 8) | (bytes[s8] & 255);
            int d2 = d + 1;
        ...
        if (left2 > 0) {
            int i4 = ((bytes[eLen] & BLEServiceApi.CONNECTED_STATUS) << 10) | (left2 == 2 ? (bytes[bytesLen - 1] & BLEServiceApi.CONNECTED_STATUS) << 2 : 0);
