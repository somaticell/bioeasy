...
public class CustomViewAbove extends ViewGroup {
    ...
    public float getPercentOpen() {
        if (this.mViewBehind.getMode() != 2) {
            return Math.abs(this.mScrollX - ((float) this.mContent.getLeft())) / ((float) getBehindWidth());
        }
        float offset = this.mScrollX - ((float) this.mContent.getLeft());
        if (offset > 0.0f) {
            return offset / ((float) getSecondaryBehindWidth());
        }
        if (offset < 0.0f) {
            return Math.abs(offset) / ((float) getBehindWidth());
        }
        return Math.abs(this.mScrollX - ((float) this.mContent.getLeft())) / ((float) getBehindWidth());
    }
    ...
    public int getSecondaryBehindWidth() {
        ...
        return this.mViewBehind.getSecondaryBehindWidth();
    }
}
